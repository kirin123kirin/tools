...
- `iro`は、資料の配色を「深緑×レンガ色×グレー」の対比配色に統一
  ...
  1. **既存スライドの独自色化**: 全スライド上の全シェイプ（グループ内も
     ...
     既存スライドの見た目は変わりません
  2. **テーマカラーの変更**: アクセント1〜6を新配色（`#00B258`の緑、
     `#CC0033`のレンガ色、`#808080`のグレーとそれぞれの薄いバリエー
...
- `bunkatsu`は、1枚の画像の中に写っている複数の物体（重なっていない、
  ...
  1. 選択中の画像シェイプをPNGとしてエクスポートし、前景/背景を判定した
     上で距離変換 + watershedで物体ごとに領域分割します。前景/背景の
     判定は、`touka`の出力等アルファチャンネルが十分にある画像は
     それをそのまま使い、写真や通常の図形のような不透過画像は大津の
     二値化にフォールバックします（塗りつぶしなしの図形も、輪郭を
     一度塗りつぶしてから検出するため分割対象になります）
  ...
  - `--distance-ratio`（既定0.15、0〜1）で分割の厳しさを調整できます。

...
入力パターンごとに、出力ファイルのデフォルトの保存先が異なります。

| 入力パターン | デフォルトの保存先 |
|---|---|
| ファイルパス指定 | 元ファイルと同じ場所に `{元のファイル名}_<コマンド名>.png` |
| クリップボードのファイルオブジェクト（Explorerでコピーした実ファイル） | 同上（コピー元ファイルと同じ場所） |
| クリップボードの画像データ（画像ソフトの「コピー」等、元ファイルなし） | カレントディレクトリに `clipboard_<コマンド名>_{タイムスタンプ}.png` |

「元ファイルが分かるか」で振り分けているため、Explorerでファイルをコピーした場合はパス指定と同じ扱いになり、元ファイルの隣に結果が保存されます。
